...
import '../../styles/layout/InputForm.scss'
...
function InputForm ({ tasks, newTask, filteredTasks, createNewTask, addNewTask, filterStatusTask }) {
  ...
  return (
    <InputGroup size='md'>
      ...
      <label htmlFor='task' />
      <InputLeftElement pointerEvents='none'> <CheckIcon color={tasks.find(item => item.name.toLowerCase().includes(newTask.name.toLowerCase())) || newTask.name === '' ? 'gray.300' : 'green.300'} /> </InputLeftElement>
